...
class FuelCardStats extends StatelessWidget {
  ...
  const FuelCardStats({super.key, required this.fuelCards});
  ...
  Map<String, dynamic> _calculateStats() {
    ...
    final activeCards =
        fuelCards.where((card) => card.status == 'active').length;
    final assignedCards =
        fuelCards.where((card) => card.status == 'assigned').length;
    final totalBalance = fuelCards.fold<double>(
      0,
      (sum, card) => sum + card.currentBalance,
    );
    final totalLimit = fuelCards.fold<double>(
      0,
      (sum, card) => sum + card.spendingLimit,
    );
    ...
    return {
      ...
      'availableCards':
          fuelCards.where((card) => card.status == 'available').length,
      'totalBalance': totalBalance,
      'totalLimit': totalLimit,
      'utilizationRate':
          totalLimit > 0 ? ((totalLimit - totalBalance) / totalLimit * 100) : 0,
    };
  }
      ...
      children: [
        ...
        Expanded(
          child: _buildStatCard(
            context,
            'Balance',
            '\$${stats['totalBalance'].toStringAsFixed(0)}',
            Icons.account_balance_wallet,
            Colors.teal,
          ),
        ),
